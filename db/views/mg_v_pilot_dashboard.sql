...
-- Vue mg.v_pilot_dashboard -- Sprint 19 -- 4 juin 2026
CREATE OR REPLACE VIEW mg.v_pilot_dashboard AS
SELECT
    ...
    (SELECT ROUND(AVG(response_time_ms),1) FROM mg.api_usage_registry WHERE request_timestamp >= now()-interval '7 days') AS temps_moyen_reponse_ms,
    ...
    (SELECT ROUND(100.0*COUNT(*) FILTER (WHERE status='RESOLU')/NULLIF(COUNT(*),0),1) FROM mg.pilot_tickets) AS taux_resolution,
    (SELECT ROUND(AVG(EXTRACT(EPOCH FROM (resolved_at-created_at))/3600),1) FROM mg.pilot_tickets WHERE status='RESOLU') AS temps_moyen_resolution_h,
    ...
    now() AS generated_at;
COMMENT ON VIEW mg.v_pilot_dashboard IS 'Vue tableau de bord pilote OSA -- D2.4. Sprint 19 -- 4 juin 2026.';