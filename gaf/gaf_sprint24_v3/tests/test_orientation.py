...
sys.path.insert(0, str(Path(__file__).resolve().parents[2]) + '/gaf_v3')

# Import adaptatif : gaf_v3 en local, gaf en production
import importlib, sys as _sys
_parent = str(Path(__file__).resolve().parents[1])
if _parent not in _sys.path:
    _sys.path.insert(0, str(Path(__file__).resolve().parents[1].parent))
from gaf_v3.core.orientation_engine import OrientationEngine
from gaf_v3.core.gaf_ledger import compute_finding_hash
...
def test_unclassified():
    ...
    assert r.get("publication_impact") == "NONE"
